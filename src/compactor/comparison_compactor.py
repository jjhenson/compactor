class AssertDummy:
    @staticmethod
    def format(msg: str, expect: str, actual:str) -> str:
        return msg + " Expected:" + expect + ", got:" + actual
...
class ComparisonCompactor:
    ...
    def _compact_string(self, source: str) -> str:
        result = self.DELTA_START + source[self.f_prefix : - self.f_suffix + 1] + self.DELTA_END
        if self.f_prefix > 0:
    ...
    def _find_common_prefix(self):
        try:
            self.f_prefix = next(i for i, (a1, a2) in enumerate(zip(self.f_expected, self.f_actual)) if a1 != a2)
    ...
    def _find_common_suffix(self):
        ...
        try:
            self.f_suffix = next(i for i in range(1, min_len) if self.f_expected[-i] != self.f_actual[-i])
        except StopIteration:
            self.f_suffix = min(len(self.f_expected), len(self.f_actual))
    ...
    def _compute_common_suffix(self):
        ...
        return self.f_expected[-self.f_suffix + 1: end] + \
